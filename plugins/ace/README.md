...
`hooks/ace-stop-capture-nudge.sh` is registered as a bounded Stop hook with `timeout: 10`. It stays local and deterministic: it reads only the local transcript path from Claude Code's hook payload, uses line/tool-count proxies, and emits the fixed nudge on stderr with exit 2 under an `asyncRewake: true` registration (rendered by Claude Code as `Stop hook feedback`, not an error, with the model woken to offer capture — a plain command hook would render the same output as `Stop hook error`) at most twice per session: once when the transcript first looks substantive, and once more only if the transcript later grows to at least 3x its size at the first nudge (long sessions accumulate the real capsule material late). The cross-session cooldown gates first nudges; the growth re-arm is same-session only. Disable it per profile/session with `ACE_CAPTURE_NUDGE=0` (also accepts `false`, `no`, or `off`) or remove the Stop entry from `hooks/hooks.json`. Do not add stale-draft or admin-queue nudges without a separate founder gate — those are deferred Phase C/D.

**Headless / CI:** disable the nudge for every print-mode invocation, for example `ACE_CAPTURE_NUDGE=0 claude -p "..."`. When the default-on Stop hook fires in `claude -p` (exit 2 re-engages the model; in print mode the asyncRewake registration may run synchronously), Claude's post-hook continuation can replace the task answer in stdout; that makes the default posture unsafe for scripted stdout consumers even though interactive sessions render both messages.
