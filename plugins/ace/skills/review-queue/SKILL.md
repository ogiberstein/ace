...
2. For each row, show Hermes verdict, cleaned-vs-original summary, and `verdict_version`.
3. Approve only exact cleaned bytes that pass policy. Call `POST /v1/submissions/:id/decision` with `{ "action": "approve", "verdict_version": <version seen> }`.
